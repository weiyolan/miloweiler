...
export default function ProjectPicture({ images, visibleItem, handleVisibility, nextVisibility, prevVisibility, index }) {
  ...
  // }
  let ar = images[visibleItem.indexOf(true) || 0].asset.metadata.dimensions.aspectRatio;
  // let ar=0;
  // console.log(images[visibleItem.indexOf(true) || 0])

  return (
    // <Link href={`./gallery/${project.slug.current}`}>
    <div className={`project-picture-container relative w-full md:w-[70%] flex h-[calc(100%-80px)]  overflow-visible justify-between items-center px-4 md:px-14 select-none`}
    // onMouseUp={mouseUp}
    // onMouseDown={mouseDown}
    // onMouseEnter={mouseEnter}
    // onMouseLeave={mouseLeave}
    >
      {/* {console.log(images[visibleItem.indexOf(true)||0].asset.metadata.dimensions.aspectRatio)} */}
      {/* {console.log(images[visibleItem.indexOf(true)||0].asset.metadata.palette.darkMuted.background)} */}
...
function MainPicture({ visible, image, alt, prevVisibility, nextVisibility, index }) {
  ...
  return (
    ...
    <div className={`absolute flex items-center justify-center max-w-[700px] w-[80vw] md:w-[50vw] h-[350px] md:h-[550px]  top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 `}>
      ...
      <SanityImage sizes='(max-width: 700px) 100vw, (max-width: 1500px) 50vw, 33vw' style={{ objectFit: 'contain' }} className={` mainPicture-${index}  opacity-0 invisible`} image={image} alt={alt} />
      {/* ${!index ? 'opacity-1 visible' : 'opacity-0 invisible'} */}
