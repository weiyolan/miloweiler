...
export default function ProjectPicture({ images, visibleItem, handleVisibility, nextVisibility, prevVisibility, index }) {
  ...
  return (
    ...
    <div className={`relative w-full md:w-[55%] h-[calc(100%-80px)] flex overflow-visible justify-between items-center px-4 md:px-16 select-none`}
    // onMouseUp={mouseUp}
    // onMouseDown={mouseDown}
    // onMouseEnter={mouseEnter}
    // onMouseLeave={mouseLeave}
    >
      <AiFillCaretLeft key={'left'} onClick={prevVisibility} className='relative  h-3/5 drop-shadow-xl cursor-pointer w-4 md:w-10 md:px-2.5 z-[1] ' />
      ...
      <AiFillCaretRight key={'right'} onClick={nextVisibility} className='relative  h-3/5 drop-shadow-xl cursor-pointer w-4 md:w-10 md:px-2.5 z-[1]' />
    </div>
    // </Link>
  )
}
...
function MainPicture({ visible, image, alt, prevVisibility, nextVisibility }) {
  return (
    ...
    <div className={`absolute flex items-center justify-center max-w-[700px] w-[80vw] md:w-[45vw] h-[250px] md:h-[550px]  top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 ${visible ? '' : 'invisible'}`}>
