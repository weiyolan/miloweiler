...
export default function PageDescription4({ info, link, className, animateName,shadow }) {
  ...
  return (
    <div className={`${animateName+'Container'} fixed w-[30%] text-primary visible opacity-100 ${className ? className : ''}`}>
      <div className={`${animateName + 'Inner'}  `}>
        <h2 className={`${animateName} font-lora font-bold text-5xl invisible opacity-0 ${shadow?'drop-shadow-[0_0px_20px_#FFF5EAFF] ':''}`} ><span className={`bg-blackWhite text-transparent bg-clip-text`}>{info.title}</span></h2>
