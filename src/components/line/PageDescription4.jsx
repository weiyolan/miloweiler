  ...
  return (
    <div className={`${animateName + 'Container'} fixed w-full md:w-1/2 xl:w-[30%] text-primary visible opacity-100 ${className ? className : ''}`}>
      <div className={`${animateName + 'Inner'}  `}>
        <h2 className={`${animateName} font-lora font-semibold text-3xl lg:text-5xl invisible uppercase opacity-0 ${shadow ? 'drop-shadow-[0_0px_10px_#000000FF] whitespace-nowrap  md:drop-shadow-[0_0px_20px_#000000FF] ' : ''}`} ><span className={`bg-blackWhite text-transparent bg-clip-text`}>{info?.title?.[locale] || ''}</span></h2>
