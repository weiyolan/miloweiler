...
export default function Page3Photos({ className, animateName }) {
  ...
  // useEffect(() => {
  //   gsap.utils.toArray(".page3photos").forEach(photo => {
  ...
  return (
    <div className={'page3photosContainer fixed w-screen h-screen'} >
      <div className='w-full h-full page3photosContainerInner'>
        <PageImage n={0} alt='' src='/images/mainpageArt1.jpg' className={`top-[55%] left-[75%] scale-[1.09] -translate-x-1/2 `} />
        <PageImage n={1} alt='' src='/images/mainpageArt2.jpg' className={`top-[38%] left-[48%] scale-[0.82] -translate-x-1/2 `} />
        <PageImage n={2} alt='' src='/images/mainpageArt3.jpg' className={`top-[20%] left-[23%] scale-[0.98] -translate-x-1/2 `} />
      </div>
    </div>
  )
}
  ...
  return (
    <div className={`absolute ${className}`}>
      <svg width="0" height="0" >
        <clipPath id={`svgClip${n}`} clipPathUnits="objectBoundingBox">
          <path id={`bird${n}`} d={paths[n]} fill="#0E0C0C" />
        </clipPath>
      </svg>
      <Image style={{ clipPath: `url(#svgClip${n})` }}
        width={1500} height={n === 2 ? 1000 : 2250} className='w-[20vw] select-none page3photos hover:cursor-pointer opacity-0 invisible  drop-shadow-2xl ' sizes='(max-width: 640px) 50vw, 25vw' {...props} />
    </div>
  )
}
