  ...
  return (
    <div className={`${animateName+'Container'} fixed w-[90%] sm:w-2/3 md:w-1/2  lg:w-1/3 text-primary visible opacity-100 ${className ? className : ''}`}>
      <div className={`${animateName + 'Inner'}  `}>
        <h2 className={`${animateName} ${shadow ? 'drop-shadow-[0_0px_10px_#FFF5EA88]  md:drop-shadow-[0_0px_20px_#FFF5EA88] ' : ''} font-lora font-bold text-3xl lg:text-5xl invisible opacity-0`} >{info?.title?.[locale] || ''}</h2>
