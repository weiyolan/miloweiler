  ...
  return (
    <div className={`${animateName+'Container'} fixed w-[90%] sm:w-2/3 md:w-1/2  xl:w-1/3 text-foreground visible opacity-100 ${className ? className : ''}`}>
      <div className={`${animateName + 'Inner'}  `}>
        <h2 className={`${animateName} ${shadow ? 'drop-shadow-[0_0px_10px_currentColor] md:drop-shadow-[0_0px_20px_currentColor]' : ''} font-serif font-extrabold text-6xl lg:text-5xl invisible opacity-0 `} >{info?.title?.[locale] || ''}</h2>
