...
export default function PageDescription({ info, link, className, animateName, shadow }) {
  ...
  return (
    <div className={`${animateName+'Container'} fixed w-2/3 md:w-1/2  lg:w-1/3 text-primary visible opacity-100 ${className ? className : ''}`}>
      <div className={`${animateName + 'Inner'}  `}>
        ...
        <p className={`${animateName} font-pop font-light my-3 invisible opacity-0 text-sm`}>{info.text}</p>
