...
export function PathGSAPStandalone(props) {
  ...
  const getPathProps = useCallback((props) => {
    ...
    if (props?.ignoreVisibility) {
      let myStroke = props.myGradient?props.myGradient:props?.strokeColor||'rgb(var(--color-foreground))'
      pathProps.stroke = props.animateStroke ? 'transparent' : myStroke
      pathProps.fill = props.animateFill ? props?.fillColor || 'rgb(var(--color-foreground))' : 'transparent';
    } else {
