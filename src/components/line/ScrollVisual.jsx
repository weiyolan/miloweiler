...
const ScrollVisual = ({ velocity }) => {
  ...
  return (
    <div style={{ transform: `translate(0, calc(${98 * scrolled.current}vh)` }} className={`fixed z-50 top-0 right-0 rounded-full bg-white/50 w-1 h-4 mr-1`}>
      ...
      <p style={{ fontWeight: '600' }} className='text-primary block font-sans text-md -translate-x-16 relative'>
        {`${window?.scrollY}`}
      </p>
      {/* <p style={{ fontWeight: '600' }} className='text-primary block font-sans text-md -translate-x-16 relative'>{`${scrolled.toPrecision(4)}`}</p> */}
    </div>
  )
}
