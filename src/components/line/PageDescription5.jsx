...
export default function PageDescription5({ info, link, className, animateName, transitionTl }) {
  ...
  return (
    <div className={`${animateName + 'Container'} fixed left-0 md:left-1/2 md:translate-x-0 h-screen w-full md:w-[50%] text-primary visible opacity-100 ${className ? className : ''}`}>
      <div className={`${animateName + 'Inner'} flex relative flex-col justify-end w-full h-full md:justify-center items-center gap-2 md:gap-8 px-[5%] pb-[5%] md:pb-[10%]`}>
        ...
        <LogoAnimated transitionTl={transitionTl} darkMode className={`${animateName} invisible opacity-0  w-4/5  md:min-h-0 md:w-1/2 min-[1800px]:w-[40%]`} />
        {/* Dark Background */}
        {mobile ?
          < FadeDiv type={'both'} amount={30} className={`absolute bottom-[5%] w-full h-[60%] invisible opacity-0 bg-darkPrimary/40   ${animateName}`}>
          </FadeDiv>
          :
          <></>}
        <p className={`${animateName} text-center w-full md:w-2/3 min-[1800px]:w-1/3 font-pop text-sm md:text-base my-3 invisible opacity-0`}>{info.text}</p>
