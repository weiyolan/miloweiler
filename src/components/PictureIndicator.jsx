...
export default function PictureIndicator({ handleVisibility, visibleItem ,setPosition}) {
  let indicatorRef = useRef(null)
  // let [indicatorPosition,setIndicatorPosition] = useState(undefined)
  let {width,height,top,bottom} = useDimensions(indicatorRef, {margin:true})

  useEffect(()=>{
    setPosition({width,height,top,bottom})
  },[bottom])
  ...
  return (
    ...
    <div ref={indicatorRef} className='picture-indicator relative w-fit inline-flex mx-auto justify-center items-center gap-2 my-4 mobl:my-8'>
