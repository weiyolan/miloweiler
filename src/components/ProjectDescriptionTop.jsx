...
export default function ProjectDescriptionTop({ project }) {
  ...
  return (
    <div className="description-container absolute w-full md:w-[70%] bottom-full left-0 px-1 md:px-12 z-[2]">
      <div className='relative w-full bg-black/40 shadow-2xl backdrop-blur cursor-pointer translate-y-16 rounded-b-[36px] md:rounded-b-[40px] pb-4 px-4 md:px-10 pt-10 '
      ...
      >
        ...
        <div className="flex flex-col md:flex-row gap-4 relative font-lora ">
          ...
          <div className="w-full md:w-2/3 text-lg font-pop font-extralight text-justify  whitespace-pre-wrap">
            ...
            <p className='text-sm md:text-sm first-letter:float-left first-letter:text-4xl first-letter:pr-2 first-letter:font-normal first-letter:uppercase first-letter:font-lora'>{project?.description?.[locale] || ''}</p>
          </div>
        </div>

        <div className={`relative font-lora text-lg md:text-3xl mt-4 flex justify-between ${darkMode ? 'text-primary' : 'text-darkPrimary'}`}>
          <h1 className={``}>
            {project.title}
...
function Span({ text, detail }) {
  return <span className={`font-pop font-extralight ${detail ? 'text-sm' : 'text-xs'}`}>{text}</span>
}
