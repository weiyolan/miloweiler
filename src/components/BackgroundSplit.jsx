      ...
      <FadeDiv style={{ transform: "translate3d(0, 0, 0)" }} className={`w-full relative ${height ? height : 'h-full'} ${animationName ? animationName + 'Inner' : ''} `} type={type} amount={amount}>
        <div className={`w-full flex ${flexCol&&'flex-col'} md:flex-row relative ${height ? height : 'h-full'}  bg-gradient-to-br from-darkGrey to-darkGrey z-0 overflow-hidden`} >
          ...
          <div className='inline-flex flex-1 h-full relative'>
            {src1 && <Image alt='' fill priority={priority ? priority : false} src={src1} style={{ transform: "translate3d(0, 0, 0)" }} className={`will-change-transform object-cover object-center `} sizes="50vw" quality={80} onLoad={(priority && setPageLoaded !== undefined) ? () => setPageLoaded(true) : false} />}
          </div>
          <div className='inline-flex flex-1 h-full relative'>
            {src2 && <Image alt='' fill priority={priority ? priority : false} src={src2} style={{ transform: "translate3d(0, 0, 0)" }} className={`will-change-transform object-cover object-center `} sizes="50vw" quality={80} onLoad={(priority && setPageLoaded !== undefined) ? () => setPageLoaded(true) : false} />}
