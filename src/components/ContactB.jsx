  ...
  return (
    ...
    <div className={`inline-flex gap-3 mt-4 sm:mt-4 ${className?className:''}`}>
      ...
      <Link className='focus:outline-none group' title={`${locale === 'en' ? 'Send WhatsApp Message' : "Envoyer un message WhatsApp"}`} target='_blank' rel="noopener noreferrer" href={whatsappLink}>
        <Button ext={true} >
          <svg className={`w-4 sm:w-4 ${darkMode ? 'fill-primary' : 'fill-black'}`} viewBox="0 0 18 18" alt={`${locale === 'en' ? 'Send WhatsApp Message' : "Envoyer un message WhatsApp"}`} >
      ...
      <Link className='focus:outline-none group' title={`${locale === 'en' ? 'Send an Email' : "Envoyer un courriel"}`} target='_blank' rel="noopener noreferrer" href={mailLink}>
        <Button ext={true}>
          <svg className={`w-4 sm:w-4 ${darkMode ? 'fill-primary' : 'fill-black'}`} alt={`${locale === 'en' ? 'Send an Email' : "Envoyer un courriel"}`} viewBox="0 0 40 40" xmlns="http://www.w3.org/2000/svg">
...
function Button({ children }) {
  return (
    <div className={`rounded-full bg-black/30 p-1.5 border-2 border-transparent focus:outline-none cursor-pointer
