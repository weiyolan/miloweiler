...
export default function ContactB({ className, portfolioLink }) {
  const { locale, businessInfo } = useAppContext()
  ...
  const whatsappLink = `https://wa.me/${businessInfo?.whatsappNumber || ''}?text=${whatsappMsg}`;
  ...
  return (
    <div className={`flex gap-3 mt-4 sm:mt-4 ${className ? className : ''}`}>
      <Link className='focus:outline-none group' href={phoneLink} title={`${locale === 'en' ? 'Call phone number' : locale === 'nl' ? 'Telefoonnummer bellen' : "Appeler le numéro téléphone"}`}>
        <Button>
          <svg className={`w-4 sm:w-4 fill-foreground`} alt={`${locale === 'en' ? 'Call phone number' : "Appeler le numéro téléphone"}`} viewBox="0 0 27 27" xmlns="http://www.w3.org/2000/svg">
      ...
      <Link className='focus:outline-none group' title={`${locale === 'en' ? 'Send WhatsApp Message' : locale === 'nl' ? 'WhatsApp-bericht sturen' : "Envoyer un message WhatsApp"}`} target='_blank' rel="noopener noreferrer" href={whatsappLink}>
        <Button >
          <svg className={`w-4 sm:w-4 fill-foreground`} viewBox="0 0 18 18" alt={`${locale === 'en' ? 'Send WhatsApp Message' : "Envoyer un message WhatsApp"}`} >
      ...
      <Link className='focus:outline-none group' title={`${locale === 'en' ? 'Send an Email' : locale === 'nl' ? 'E-mail sturen' : "Envoyer un courriel"}`} target='_blank' rel="noopener noreferrer" href={mailLink}>
        <Button>
          <svg className={`w-4 sm:w-4 fill-foreground`} alt={`${locale === 'en' ? 'Send an Email' : "Envoyer un courriel"}`} viewBox="0 0 40 40" xmlns="http://www.w3.org/2000/svg">
      ...
      <Link className='focus:outline-none group' title={`${locale === 'en' ? 'Download my portfolio' : locale === 'nl' ? 'Download mijn portfolio' : "Téléchargé mon portfolio"}`} href={`${portfolioLink}`}>
        <Button>
          <svg className={`w-4 sm:w-4 fill-foreground`} alt={`${locale === 'en' ? 'Download my portfolio' : "Téléchargez mon portfolio"}`} viewBox="0 0 17 17" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path d="M13.7734 10.7071L9.18758 15.2929C8.79705 15.6834 8.16389 15.6834 7.77336 15.2929L3.18758 10.7071C2.55761 10.0771 3.00378 9 3.89468 9H13.0663C13.9572 9 14.4033 10.0771 13.7734 10.7071Z" fill="currentColor" />
            <path d="M8.48047 3V10" stroke="currentColor" strokeWidth="3" strokeLinecap="round" />
          </svg>
        </Button>
      </Link>
    </div>
...
function Button({ children }) {
  return (
    <div className={`rounded-full bg-foreground/10 hover:border-foreground/90 border  p-1.5  border-transparent focus:outline-none cursor-pointer
