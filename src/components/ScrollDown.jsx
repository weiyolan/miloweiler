...
export default function ScrollDown({ className, style, ctx }) {
  ...
  useEffect(() => {
    ...
    function handleScroll() {
      if (scrolled.current > 0.005 && !down) {
        setDown(true)
        ctx.current.add(() => {
  ...
  return (
    ...
    <div style={style} className={`${className && className}`} onClick={() => { gsap.to(window, { duration: 0.5, scrollTo: 0.90 * height }) }}>
      ...
      <span className="text-foreground font-serif scrollDownInner opacity-0" >{locale === 'fr' ? 'défilez' : 'scroll down'}</span>
      <svg className='scrollDownInner opacity-0' width="12" height="23" viewBox="0 0 12 23" fill="none" xmlns="http://www.w3.org/2000/svg">
        <path id="Vector 121" d="M5.50157 1.5C5 6 7.5 6.5 7.5 9C7.5 11.5 4.5 11 4.5 13C4.5 15 6.5 15.5 6 20.5L1 17L6 22L10.5 17L6 20" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
      </svg>
    </div>
  )
}
