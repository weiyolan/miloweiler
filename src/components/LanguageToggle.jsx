...
export default function LanguageToggle({ toggleNav }) {
  ...
  return (
    <div
      ...
      // onClick={()=>toggleNav()}
      className={`absolute md:fixed navButton top-0 right-0 text-foreground text-2xl xs:text-xl sm:text-base text-center flex justify-center items-center font-serif
    font-black  border border-transparent
    cursor-pointer select-none m-2 md:m-4 lg:m-5 `}>
      {/* self-center inline-flex */}


      <TfiWorld className={`relative inline-flex md:absolute left-1/2 -translate-x-1/2 top-1/2 -translate-y-1/2 w-5 h-5 languageIcon text-foreground`} />
