    ...
    <form onSubmit={handleSubmit} name='ContactForm' method="POST" data-netlify="true" netlify-honeypot="bot-field" className='flex relative flex-col items-start' >
      ...
      <div className={`grid gap-1 xs:gap-6 grid-cols-3 w-full min-w-[30vw] auto-rows-min font-normal placeholder:text-xs min-[400px]:placeholder:text-sm ${darkMode ? 'text-primary placeholder:text-primary' : 'text-black placeholder:text-black'}`}>
        <div className='inline-flex col-start-1 col-span-3 min-[500px]:col-span-2'>
          <div className='inline-block relative col-start-1 col-span-1 pr-3'>
            ...
            <input required name='name' className={`block bg-black/10 
              ...
              focus:-outline-offset-2 focus:outline-black/20 p-2 w-full text-sm my-2`} id='name'
              type='text'
          ...
          <div className='inline-block relative col-start-2 col-span-1 pl-3'>
            <label className=' cursor-pointer whitespace-nowrap font-semibold inline-flex max-w-fit text-xs mt-2 ml-1' htmlFor='lastname'>{`${locale === 'en' ? 'Last Name' : "Nom"}`}</label>
            <input required name='lastname'
              ...
              border-none border-transparent invalid:text-red-700
              placeholder:text-black/50
              focus:-outline-offset-2 focus:outline-black/20 p-2 w-full text-sm my-2`}
              id='lastname'
              type="text"
        ...
        <div className='inline-block relative col-start-1 col-span-3 min-[400px]:col-span-2'>
          <label className=' cursor-pointer whitespace-nowrap font-semibold text-xs inline-flex max-w-fit mt-2 ml-1' htmlFor='email'>{`${locale === 'en' ? 'Email' : "Email"}`}</label>
          <input required name='email' className={`block bg-black/10 
              rounded-full  autofill:bg-black/10 valid:scale-[0.99] 
              outline-none -outline-offset-2 focus:outline-none focus:animate-outlinePulse
              border-none border-transparent invalid:text-red-700
              placeholder:text-black/50
              focus:-outline-offset-2 focus:outline-black/20 p-2 w-full text-sm my-2`} id='email'
            type='email'
        ...
        <div className='inline-flex flex-col w-full col-start-1 col-span-3 '>
          <label className=' cursor-pointer whitespace-nowrap font-semibold text-xs inline-flex max-w-fit mt-2 ml-1' htmlFor='message'>{`${locale === 'en' ? 'Message' : "Message"}`}</label>
          <textarea required className={`block bg-black/10 
                autofill:bg-black/10 valid:scale-[0.99] 
              outline-none -outline-offset-2 focus:outline-none focus:animate-outlinePulse
              border-none border-transparent invalid:text-red-700
              placeholder:text-black/50
              focus:-outline-offset-2 focus:outline-black/20 p-2 w-full text-sm my-2 h-[20vh] rounded-3xl`} id='message'
            type='text'
            name='message'
            placeholder={
