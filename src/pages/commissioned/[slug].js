  ...
  return (
    <>
      <Head>
        ...
        <meta property="og:url" content={`https://miloweiler.com/${locale === "en" ? "" : locale + "/"}/commissioned/${project.slug.current}`} />
        <meta property="fb:app_id" content="659504862954849" />
        {/* TWITTER */}
        <meta name="twitter:card" content="summary_large_image" />
        <meta property="twitter:domain" content="miloweiler.com" />
        <meta property="twitter:url" content={`https://miloweiler.com/${locale === "en" ? "" : locale + "/"}/commissioned/${project.slug.current}`} />
...
export async function getStaticProps({ params }) {
  ...
  const [project, projectSlugs] = await Promise.all([
    ...
    client.fetch(`*[_type == "project" && commissionedBool == true]|order(date desc){slug}`),
  ]);
