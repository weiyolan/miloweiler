      ...
      if (Array.isArray(projectImages) && projectImages.length > 0) {
        return (
          projectImages
            ...
            .flatMap((positionedImage) => {
              // avoid carrying over the array _key
              const {_key, position, ...props} = positionedImage
              // console.log(_key)
              return [
                patch(
                  doc._id,
                      ...
                      [
                        {
                          ...
                          _type: 'object',
                          ...props,
                          // image:{_type:'metaImage'},
                          position: {
                            lg: {x: 1, width: 1, y: 1, height: 1},
                            sm: {x: 1, width: 1, y: 1, height: 1},
                          },
                        },
                      ],
                      {_key}
